...
var readability = {
		...
		xhr.onreadystatechange = function() {
			if ( xhr.readyState == 4 ) {
				if ( xhr.status == 200 ) {
					...
					if ( arr_match !== null ) {
						readability.init(arr_match[1].decodeURIComponent());
					}
					else {
				        ...
				        var articleTitle   = readability.getArticleTitle(element);
    ...
    **/
     ...
     * @return void
     **/
    getArticleTitle: function ( element ) {
        var curTitle = "",
            origTitle = "";

        try {
            curTitle = origTitle = element.title;
            
            if(typeof curTitle !== "string") { /* If they had an element with id "title" in their HTML */
                curTitle = origTitle = readability.getInnerText(element.getElementsByTagName('title')[0]);             
            }
        }
        catch(e) {}
        ...
        {
            var hOnes = element.getElementsByTagName('h1');
            if(hOnes.length === 1)
            {
