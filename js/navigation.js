	...
	content.xhr("data/timeline.html", {
	    successCallback: function() {
	    	...
	    	*/
	    	...
	    	//str_timeline = str_timeline + '<tablecell id="more_timeline" onclick="doRefresh(last_tweet_id, true);" style="text-align: center; height: 45px;">Load more tweets</tablecell>';
...
// Navigate to the top of the timeline.
function goHome( time ) {
	/*
	var uiscroller = $.UIScrollers[whichScroller];
	if (time === undefined) time = 200;
	uiscroller.scrollTo(0, 0, time);
	untouchButtons();
	*/
	1;
}
...
function postTweet() {
	...
	oauth.post(url, 
		{ 'status': status, 'in_reply_to_status_id': in_reply_to_status_id, 'trim_user': '1', 'include_entities': '0' },
